...
@_implementationOnly import CoreCryptoSwift
import Foundation
...
private extension CoreCryptoSwift.CommitBundle {
...
private extension CoreCryptoSwift.MemberAddedMessages {
...
private extension CoreCryptoSwift.ConversationInitBundle {
...
private extension CoreCryptoSwift.DecryptedMessage {
...
private extension CoreCryptoSwift.WireIdentity {
...
private extension CoreCryptoSwift.ProposalBundle {
    func convertTo() -> ProposalBundle {
        return ProposalBundle(proposal: self.proposal, proposalRef: self.proposalRef)
    }
}

private extension CoreCryptoSwift.PublicGroupStateBundle {
...
private extension CoreCryptoSwift.MlsPublicGroupStateEncryptionType {
...
private extension CoreCryptoSwift.ProteusAutoPrekeyBundle {
    func convertTo() -> ProteusAutoPrekeyBundle {
        return ProteusAutoPrekeyBundle(id: self.id, pkb: self.pkb)
    }
}

private extension CoreCryptoSwift.MlsRatchetTreeType {
...
public struct DecryptedMessage: ConvertToInner {
    ...
    public var senderClientId: [UInt8]?
    ...
    public init(message: [UInt8]?, proposals: [ProposalBundle], isActive: Bool, commitDelay: UInt64?, senderClientId: [UInt8]?, hasEpochChanged: Bool, identity: WireIdentity?) {
...
private extension CoreCrypto {
    var coreCrypto: CoreCryptoWrapper.WrappedCoreCrypto {
        CoreCryptoWrapper.WrappedCoreCrypto(coreCrypto: self)
    }
}

private extension WireE2eIdentity {
    var wireE2EIdentity: CoreCryptoWrapper.WireE2EIdentity {
        CoreCryptoWrapper.WireE2EIdentity(wireE2EIdentity: self)
    }
}
...
public class CoreCryptoWrapper {
    ...
    public class WrappedCoreCrypto {

        internal init(coreCrypto: CoreCryptoSwift.CoreCrypto) {
            self.swiftCrypto = coreCrypto
        }

        let swiftCrypto: CoreCryptoSwift.CoreCrypto
    }

    public class WireE2EIdentity {
        internal init(wireE2EIdentity: WireE2eIdentity) {
            self.wireE2EIdentity = wireE2EIdentity
        }

        let wireE2EIdentity: CoreCryptoSwift.WireE2eIdentity
    }

    public class CoreCryptoCallbacks {
        internal init(coreCryptoCallbacks: CoreCryptoSwift.CoreCryptoCallbacks) {
            self.coreCryptoCallbacks = coreCryptoCallbacks
        }

        var coreCryptoCallbacks: CoreCryptoSwift.CoreCryptoCallbacks
    }
    
    fileprivate let coreCrypto: CoreCryptoWrapper.WrappedCoreCrypto
    ...
    ///
    public init(path: String, key: String, clientId: [UInt8], entropySeed: [UInt8]?) throws {
        self.coreCrypto = try CoreCryptoSwift.CoreCrypto(path: path, key: key, clientId: clientId, entropySeed: entropySeed).coreCrypto
    }
    ...
    public static func deferredInit(path: String, key: String, entropySeed: [UInt8]?) throws -> CoreCryptoWrapper.WrappedCoreCrypto {
        try CoreCryptoSwift.CoreCrypto.deferredInit(path: path, key: key, entropySeed: entropySeed).coreCrypto
    }

    /// Use this after ```CoreCrypto/deferredInit``` when you have a clientId. It initializes MLS.
    ///
    /// - parameter clientId: client identifier
    public func mlsInit(clientId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.mlsInit(clientId: clientId)
    }
    ...
    public func mlsGenerateKeypair() throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.mlsGenerateKeypair()
    }
    ...
    public func mlsInitWithClientId(clientId: [UInt8], signaturePublicKey: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.mlsInitWithClientId(clientId: clientId, signaturePublicKey: signaturePublicKey)
    }
    ...
    public func restoreFromDisk() throws {
        return try self.coreCrypto.swiftCrypto.restoreFromDisk()
    }
    ...
    public func setCallbacks(callbacks: CoreCryptoCallbacks) throws {
        try self.coreCrypto.swiftCrypto.setCallbacks(callbacks: callbacks.coreCryptoCallbacks)
    }

    /// - returns: The client's public key
    public func clientPublicKey() throws -> [UInt8] {
        return try self.coreCrypto.swiftCrypto.clientPublicKey()
    }
    ...
    public func clientKeypackages(amountRequested: UInt32) throws -> [[UInt8]] {
        return try self.coreCrypto.swiftCrypto.clientKeypackages(amountRequested: amountRequested)
    }

    /// - returns: The amount of valid, non-expired KeyPackages that are persisted in the backing storage
    public func clientValidKeypackagesCount() throws -> UInt64 {
        return try self.coreCrypto.swiftCrypto.clientValidKeypackagesCount()
    }
    ...
    public func createConversation(conversationId: [UInt8], config: ConversationConfiguration) throws {
        try self.coreCrypto.swiftCrypto.createConversation(conversationId: conversationId, config: config.convert())
    }
    ...
    public func conversationExists(conversationId: [UInt8]) -> Bool {
        return self.coreCrypto.swiftCrypto.conversationExists(conversationId: conversationId)
    }

    /// Returns the epoch of a given conversation id
    /// - parameter conversationId: conversation identifier
    /// - returns: the current epoch of the conversation
    public func conversationEpoch(conversationId: [UInt8]) throws -> UInt64 {
        return try self.coreCrypto.swiftCrypto.conversationEpoch(conversationId: conversationId)
    }
    ...
    public func processWelcomeMessage(welcomeMessage: [UInt8], configuration: CustomConfiguration) throws -> [UInt8] {
        return try self.coreCrypto.swiftCrypto.processWelcomeMessage(welcomeMessage: welcomeMessage, customConfiguration: configuration.convert())
    }
    ...
    public func addClientsToConversation(conversationId: [UInt8], clients: [Invitee]) throws -> MemberAddedMessages {
        return try self.coreCrypto.swiftCrypto.addClientsToConversation(conversationId: conversationId, clients: clients.map({ (invitee) -> CoreCryptoSwift.Invitee in
    ...
    /// - returns: A ``CommitBundle`` byte array to fan out to the Delivery Service
    public func removeClientsFromConversation(conversationId: [UInt8], clients: [[UInt8]]) throws -> CommitBundle {
        return try self.coreCrypto.swiftCrypto.removeClientsFromConversation(conversationId: conversationId, clients: clients).convertTo()
    }
    ...
    public func markConversationAsChildOf(childId: [UInt8], parentId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.markConversationAsChildOf(childId: childId, parentId: parentId)
    }
    ...
    /// - parameter conversationId: conversation identifier
    /// - returns: A ``CommitBundle`` byte array to fan out to the Delivery Service
    public func updateKeyingMaterial(conversationId: [UInt8]) throws -> CommitBundle {
        try self.coreCrypto.swiftCrypto.updateKeyingMaterial(conversationId: conversationId).convertTo()
    }
    ...
    /// - parameter conversationId: conversation identifier
    /// - returns: A ``CommitBundle`` byte array to fan out to the Delivery Service
    public func commitPendingProposals(conversationId: [UInt8]) throws -> CommitBundle? {
        try self.coreCrypto.swiftCrypto.commitPendingProposals(conversationId:conversationId)?.convertTo()
    }

    /// Destroys a group locally
    ///
    /// - parameter conversationId: conversation identifier
    public func wipeConversation(conversationId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.wipeConversation(conversationId: conversationId)
    }
    ...
    public func decryptMessage(conversationId: [UInt8], payload: [UInt8]) throws -> DecryptedMessage {
        return try self.coreCrypto.swiftCrypto.decryptMessage(conversationId: conversationId, payload: payload).convertTo()
    }
    ...
    public func encryptMessage(conversationId: [UInt8], message: [UInt8]) throws -> [UInt8] {
        return try self.coreCrypto.swiftCrypto.encryptMessage(conversationId: conversationId, message: message)
    }
    ...
    public func newAddProposal(conversationId: [UInt8], keyPackage: [UInt8]) throws -> ProposalBundle {
        return try self.coreCrypto.swiftCrypto.newAddProposal(conversationId: conversationId, keyPackage: keyPackage).convertTo()
    }
    ...
    /// - parameter conversationId: conversation identifier
    /// - returns: a message (to be fanned out) will be returned with the proposal that was created
    public func newUpdateProposal(conversationId: [UInt8]) throws -> ProposalBundle {
        return try self.coreCrypto.swiftCrypto.newUpdateProposal(conversationId: conversationId).convertTo()
    }
    ...
    /// - returns: a message (to be fanned out) will be returned with the proposal that was created
    public func newRemoveProposal(conversationId: [UInt8], clientId: [UInt8]) throws -> ProposalBundle {
        return try self.coreCrypto.swiftCrypto.newRemoveProposal(conversationId: conversationId, clientId: clientId).convertTo()
    }
    ...
    public func newExternalAddProposal(conversationId: [UInt8], epoch: UInt64) throws -> [UInt8] {
        return try self.coreCrypto.swiftCrypto.newExternalAddProposal(conversationId: conversationId, epoch: epoch)
    }
    ...
    public func newExternalRemoveProposal(conversationId: [UInt8], epoch: UInt64, keyPackageRef: [UInt8]) throws -> [UInt8] {
        return try self.coreCrypto.swiftCrypto.newExternalRemoveProposal(conversationId: conversationId, epoch: epoch, keyPackageRef: keyPackageRef)
    }
    ...
    public func joinByExternalCommit(publicGroupState: [UInt8], configuration: CustomConfiguration) throws -> ConversationInitBundle {
        try self.coreCrypto.swiftCrypto.joinByExternalCommit(publicGroupState: publicGroupState, customConfiguration: configuration.convert()).convertTo()
    }
    ...
    public func exportGroupState(conversationId: [UInt8]) throws -> [UInt8] {
        return try self.coreCrypto.swiftCrypto.exportGroupState(conversationId: conversationId)
    }
    ...
    ///
    /// - parameter conversationId: conversation identifier
    public func mergePendingGroupFromExternalCommit(conversationId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.mergePendingGroupFromExternalCommit(conversationId: conversationId)
    }
    ...
    ///
    /// - parameter conversationId: conversation identifier
    public func clearPendingGroupFromExternalCommit(conversationId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.clearPendingGroupFromExternalCommit(conversationId: conversationId)
    }
    ...
    public func exportSecretKey(conversationId: [UInt8], keyLength: UInt32) throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.exportSecretKey(conversationId: conversationId, keyLength: keyLength)
    }
    ...
    public func getClientIds(conversationId: [UInt8]) throws -> [[UInt8]] {
        try self.coreCrypto.swiftCrypto.getClientIds(conversationId: conversationId)
    }
    ...
    public func randomBytes(length: UInt32) throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.randomBytes(length: length)
    }
    ...
    public func reseedRng(seed: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.reseedRng(seed: seed)
    }
    ...
    ///
    /// - parameter conversationId: conversation identifier
    public func commitAccepted(conversationId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.commitAccepted(conversationId: conversationId)
    }
    ...
    public func clearPendingProposal(conversationId: [UInt8], proposalRef: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.clearPendingProposal(conversationId: conversationId, proposalRef: proposalRef)
    }
    ...
    ///
    /// - parameter conversation_id - the group/conversation id
    public func clearPendingCommit(conversationId: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.clearPendingCommit(conversationId: conversationId)
    }

    /// Initializes the proteus client
    public func proteusInit() throws {
        try self.coreCrypto.swiftCrypto.proteusInit()
    }
    ...
    public func proteusSessionFromPrekey(sessionId: String, prekey: [UInt8]) throws {
        try self.coreCrypto.swiftCrypto.proteusSessionFromPrekey(sessionId: sessionId, prekey: prekey)
    }
    ...
    public func proteusSessionFromMessage(sessionId: String, envelope: [UInt8]) throws -> [UInt8]{
        return try self.coreCrypto.swiftCrypto.proteusSessionFromMessage(sessionId: sessionId, envelope: envelope)
    }
    ...
    public func proteusSessionSave(sessionId: String) throws {
        try self.coreCrypto.swiftCrypto.proteusSessionSave(sessionId: sessionId)
    }
    ...
    public func proteusSessionDelete(sessionId: String) throws {
        try self.coreCrypto.swiftCrypto.proteusSessionDelete(sessionId: sessionId)
    }
    ...
    public func proteusSessionExists(sessionId: String) throws -> Bool {
        try self.coreCrypto.swiftCrypto.proteusSessionExists(sessionId: sessionId)
    }
    ...
    public func proteusDecrypt(sessionId: String, ciphertext: [UInt8]) throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.proteusDecrypt(sessionId: sessionId, ciphertext: ciphertext)
    }
    ...
    public func proteusEncrypt(sessionId: String, plaintext: [UInt8]) throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.proteusEncrypt(sessionId: sessionId, plaintext: plaintext)
    }
    ...
    public func proteusEncryptBatched(sessions: [String], plaintext: [UInt8]) throws -> [String: [UInt8]] {
        try self.coreCrypto.swiftCrypto.proteusEncryptBatched(sessionId: sessions, plaintext: plaintext)
    }
    ...
    public func proteusNewPrekey(prekeyId: UInt16) throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.proteusNewPrekey(prekeyId: prekeyId)
    }
    ...
    public func proteusNewPrekeyAuto() throws -> ProteusAutoPrekeyBundle {
        try self.coreCrypto.swiftCrypto.proteusNewPrekeyAuto().convertTo()
    }
    ...
    public func proteusLastResortPrekey() throws -> [UInt8] {
        try self.coreCrypto.swiftCrypto.proteusLastResortPrekey()
    }

    /// - returns: The ID of the Proteus last resort PreKey
    public func proteusLastResortPrekeyId() throws -> UInt16 {
        try self.coreCrypto.swiftCrypto.proteusLastResortPrekeyId()
    }
    ...
    public func proteusFingerprint() throws -> String {
        try self.coreCrypto.swiftCrypto.proteusFingerprint()
    }
    ...
    public func proteusFingerprintLocal(sessionId: String) throws -> String {
        try self.coreCrypto.swiftCrypto.proteusFingerprintLocal(sessionId: sessionId)
    }
    ...
    public func proteusFingerprintRemote(sessionId: String) throws -> String {
        try self.coreCrypto.swiftCrypto.proteusFingerprintRemote(sessionId: sessionId)
    }
    ...
    public func proteusFingerprintPrekeybundle(prekey: [UInt8]) throws -> String {
        try self.coreCrypto.swiftCrypto.proteusFingerprintPrekeybundle(prekey: prekey)
    }
    ...
    public func proteusCryptoboxMigrate(path: String) throws {
        try self.coreCrypto.swiftCrypto.proteusCryptoboxMigrate(path: path)
    }
    ...
    public func newAcmeEnrollment(clientId: String, displayName: String, handle: String, expiryDays: UInt32, ciphersuite: CiphersuiteName = CiphersuiteName.mls128Dhkemx25519Aes128gcmSha256Ed25519) throws -> CoreCryptoWrapper.WireE2EIdentity {
        if ciphersuite != CiphersuiteName.mls128Dhkemx25519Aes128gcmSha256Ed25519 {
        ...
        return try self.coreCrypto.swiftCrypto.newAcmeEnrollment(clientId: clientId, displayName: displayName, handle: handle, expiryDays: expiryDays, ciphersuite: ciphersuite.convert()).wireE2EIdentity
    }
    ...
    public func e2eiMlsInit(e2ei: CoreCryptoWrapper.WireE2EIdentity, certificateChain: String) throws {
        return try self.coreCrypto.swiftCrypto.e2eiMlsInit(e2ei: e2ei.wireE2EIdentity, certificateChain: certificateChain)
    }
