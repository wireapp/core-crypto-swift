...
public class WireCoreCrypto {
    ...
    ///
    /// **CAUTION**: ``WireCoreCrypto/commitAccepted`` **HAS TO** be called afterwards **ONLY IF** the Delivery Service responds
    ...
    ///
    /// If the Delivery Service accepts the external commit, you have to ``WireCoreCrypto/mergePendingGroupFromExternalCommit``
    /// in order to get back a functional MLS group. On the opposite, if it rejects it, you can either retry by just
    /// calling again ``WireCoreCrypto/joinByExternalCommit``, no need to ``WireCoreCrypto/clearPendingGroupFromExternalCommit``.
    /// If you want to abort the operation (too many retries or the user decided to abort), you can use
    /// ``WireCoreCrypto/clearPendingGroupFromExternalCommit`` in order not to bloat the user's storage but nothing
    ...
    /// This merges the commit generated by ``WireCoreCrypto/joinByExternalCommit``, persists the group permanently and
    ...
    /// In case the external commit generated by ``WireCoreCrypto/joinByExternalCommit`` is rejected by the Delivery Service,
